          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 py-10">
            {data?.map((service) => (
              <Link to={`/ServiceDetails/${service._id}`} key={service._id}>
                <div
                  key={service.Id}
                  className="cursor-pointer  text-white rounded-xl shadow-2xl border"
                >
                  <img className="w-full  rounded-t-xl  " src={service.image} alt="" />
                  <div className="p-4">
                    <h2 className="text-3xl font-bold py-6 font-cinzel">{service.title}</h2>
                    <p className="text-lg md:text-xl font-lora">{service.shortdescription}</p>
